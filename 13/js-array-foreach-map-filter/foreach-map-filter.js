...
function vowelCount(str){
    str = str.toLowerCase();
    const newObj = {}
    str.split('').forEach(function(char) { // split??
        if ('aeiou'.includes(char)) {
            if(!newObj[char]) newObj[char] = 0
            newObj[char] += 1
        }
    })
    return newObj;
...
function findInObj(arr, key, searchValue) {
    const newArr = arr.filter(function(value) {
        if(value[key] === searchValue) return value;
    }) 
    return newArr[0];
}
...
function removeVowels(str) {
    let newStr;
    str = str.toLowerCase();
    newStr = str.split('').filter(function(char) {
...
/*
function filterByValue(arr, key, searchValue) {
    const newArr = arr.filter(function(value) {
